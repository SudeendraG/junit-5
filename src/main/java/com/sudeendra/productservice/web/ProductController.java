package com.sudeendra.productservice.web;

import com.sudeendra.productservice.model.Product;
import com.sudeendra.productservice.service.ProductService;
import java.net.URI;
...
public class ProductController {
  ...
  public ResponseEntity<Product> createProduct(@RequestBody Product product) {
    logger.info("Creating new product with name: {}, quantity: {}", product.getName(),
        product.getQuantity());
   ...
   * @return A ResponseEntity that contains the updated product or one of the following error
   * statuses: NOT_FOUND if there is no product in the database with the specified ID CONFLICT if
   * the eTag does not match the version of the product to update INTERNAL_SERVICE_ERROR if there is
   * a problem creating the location URI
   */
   ...
   * @return A ResponseEntity with one of the following status codes: 200 OK if the delete was
   * successful 404 Not Found if a product with the specified ID is not found 500 Internal Service
